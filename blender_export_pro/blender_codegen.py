...
def _ring_records(atoms, rings, cfg: StyleConfig):
    """Per-ring records with the panel color already resolved."""
    ...
    for ring in rings or []:
        ...
        records.append({"indices": list(ring), "color": color})
    return records


def generate_script(atoms, bonds, cfg: StyleConfig, rings=None) -> str:
    ...
    ring_data = json.dumps(_ring_records(atoms, rings, cfg), indent=1)

    header = f'''"""Generated by MoleditPy — Blender Export Pro.

Run inside Blender {cfg.blender_target} (Scripting workspace > Run Script).
...
def make_ring_material(name, rgb):
    mat = bpy.data.materials.get(name)
    if mat:
        return mat
    mat = bpy.data.materials.new(name)
    ...
    if bsdf is not None:
        ...
        _set_input(bsdf, ["Alpha"], RING_OPACITY)
    if RING_OPACITY < 1.0 and hasattr(mat, "blend_method"):
        mat.blend_method = "BLEND"
    mat.diffuse_color = (rgb[0], rgb[1], rgb[2], RING_OPACITY)
    return mat
...
def create_ring_panel(coll, index, rec):
    ...
    for v in verts:
        center += v
    center /= len(verts)
    verts = [center + (v - center) * RING_SCALE for v in verts]
    ...
    if RING_THICKNESS > 0.0:
        ...
        mod.thickness = RING_THICKNESS
        mod.offset = 0.0

    mat = make_ring_material(
        "Mat_ring_%s" % "_".join("%.3f" % c for c in rec["color"]), rec["color"])
    obj.data.materials.append(mat)
    link_to(coll, obj)
    return obj
...
def generate_script_from_mol(mol, cfg: StyleConfig, selected_indices=None) -> str:
    ...
    return generate_script(atoms, bonds, cfg, rings)
